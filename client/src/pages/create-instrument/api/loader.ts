...
export const loader: LoaderFunction = async (): Promise<CreateInstrumentLoader> => {

  let instrumentTypes: string[] = [];
  console.log(`jwt: ${Jwt.extractFromLocalStorage()?.value}`);
  await axios.get<InstrumentTypes>(`${SERVER_URL}${API_INSTRUMENT_TYPES}`, {
    headers: {
  ...
  await axios.get<Materials>(`${SERVER_URL}${API_INSTRUMENT_MATERIALS}`, {
    headers: {
      Authorization: `Bearer ${Jwt.extractFromLocalStorage()?.toStringValue()}`
    }
  })
  ...
  await axios.get<Countries>(`${SERVER_URL}${API_COUNTRIES}`, {
    headers: {
      Authorization: `Bearer ${Jwt.extractFromLocalStorage()?.toStringValue()}`
    }
  })
